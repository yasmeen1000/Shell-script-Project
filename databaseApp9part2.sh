...
select_from_table() {
    tablename=$(zenity --entry --title="Enter table name" --text="Enter table name:")
    ...
    # Check if the input contains special characters or not
    if [[ ! $tablename =~ ^[a-zA-Z0-9_]+$ ]]; then
        zenity --error --title="Error" --text="Table name can only contain alphanumeric characters and underscores."
        return
    fi
    ...
    while true; do
        option=$(zenity --list --title="Select Query options" --column="Options" "Select All" "Select with Condition" "Back to Main Menu")

        case "$option" in
            "Select All")
                zenity --info --title="Selected Data" --text="${columns[*]}\n$(sed 's/:/ /g' "${tablename}/data")"
                ;;
            "Select with Condition")
                colname=$(zenity --entry --title="Enter column name" --text="Enter column name:")
                colvalue=$(zenity --entry --title="Enter column value" --text="Enter value of column:")
                ...
                if [ "$found" == true ]; then
                    ...
                    zenity --error --title="Error" --text="Column not found."
                fi
                ;;
            "Back to Main Menu")
                break
...
# Main menu loop
while true; do
    option=$(zenity --list --title="Database Management" --column="Options" "Create Database" "List Databases" "Connect To Database" "Drop Database" "Exit" --width=700 --height=500 --cancel-label="Exit")
